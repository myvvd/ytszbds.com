...
////
        ...
        if (cookie('rolecode') == 'specialist' or cookie('rolecode') == 'cityadmin') {
            ...
            if (empty(session('workscategory'))) {
                ...
                $rec = Db::name('expert_group')->where('id', $expertGroupId)->field('name,works_category,city_group')->find();
                session('workscategory', $rec['works_category']);
                session('citygroup',$rec['city_group']);
            }
            $map[] = ['gc_entry_work.works_category', 'IN', explode(',', session('workscategory'))];

            if (session('citygroup') != null){
                $map[] = ['gc_entry_work.city', 'IN', explode(',', session('citygroup'))];
            }
