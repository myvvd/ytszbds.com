...
class Enroll extends HomeBase
{
    ...
    public function save($data = [])
    {
		...
		if (true) {
            ...
            $res = $model->where('works_category', '=', $data['works_category'])->where('workcode', '<>', '')->count();
        ...
        if (($data['id']) !== null && !empty($data['id'])) {
            ...
            if ($submitto['checkstatus1'] == -1) {
				$model->where('id', $data['id'])->update(['status'=>0]);
                $data['checkstatus1'] = 0;
				unset($data['id']);
    ...
    public function detail()
    {
        ...
        $model = new EntryWork();
        $result = $model->where($map)->find()->getData();
        if (!$result) {
       ...
       // $result = $result->getData();
		...
		$result['patent'] = !empty($result['patent'])? explode(",",$result['patent']):[];
		foreach($result['patent'] as $key=>$v){
			$result['patent'.$v] = $v; 
		}
		
		$result['check1_remark2'] = unserialize($result['check1_remark2']);
		$result['check1_remark2'] = !empty($result['check1_remark2'])? implode(",",$result['check1_remark2']):[];
    ...
    public function uploadImg($fileInputName = '', $createThumb = true, $thumbWidth = 400, $thumbHeight = 400)
    {
        ...
        $allowExts = 'jpg,jpeg,png,bmp,tif,WebP,gif,pdf,ppt,pptx';
    ...
    public function createCertificate()
    {
        ...
        /*if (empty($result['certificate_cards'])) {
            $image = Image::open($doc_path.'/static/images/cert-bg.png');
            $font = $doc_path.'/static/simhei.ttf';
            $image->text($result['workcode'], $font, 18, $color = '#ffffff', $locate = Image::WATER_NORTHWEST, $offset = [30,400])->save('./certificate-cards/gc-'. $result['workcode'] .'.jpg');
            $model->where('id', $id)->update(['certificate_cards' => '/certificate-cards/gc-'. $result['workcode'] .'.jpg']);
        }*/
		
		 if (empty($result['certificate_cards'])) {
            $image = Image::open($doc_path.'/static/images/cert-bg.jpg');
            $font = $doc_path.'/static/simhei.ttf';
            $image->text($result['workcode'], $font, 22, $color = '#ffffff', $locate = Image::WATER_NORTHWEST, $offset = [795,90])->save('./certificate-cards/gc-'. $result['workcode'] .'.jpg');
            $model->where('id', $id)->update(['certificate_cards' => '/certificate-cards/gc-'. $result['workcode'] .'.jpg']);
        }
