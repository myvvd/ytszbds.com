  ...
  Tab.prototype.tabAdd = function (data) {
    ...
    if (_config.contextMenu) {
      ...
      ELEM.titleBox.find('li').on('contextmenu', function (e) {
        ...
        ul += '<li data-target="openInNewPage" title="在新页面打开"><i class="icon icon-plus" aria-hidden="true"></i> 在新页面打开</li>';
        ul += '</ul>';
        div.innerHTML = ul;
        ...
        if ($context.length > 0) {
          $context.eq(0).children('ul').children('li').each(function () {
            ...
            $that.on('click', function () {
              ...
              switch (target) {
                case 'openInNewPage': //刷新当前
                  window.open(ELEM.contentBox.find('iframe[data-id=' + id + ']')[0].src);
                  break;
                case 'refresh': //刷新当前
